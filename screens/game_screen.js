                ...
                <TouchableOpacity style={{
                        ...
                        onPress={() => { 
                                        ...
                                        if(this.props.Boardcellval == 2 && !this.props.isGenerated && this.state.to_rend_save){
                                            ...
                                            saved_generated_render_switch = false
                    ...
                    ((((this.props.solvedBoardcellval == 2 && this.props.showSolution) || (this.state.to_rend && saved_generated_render_switch)) 
...
export default class GameScreen extends React.Component {
    ...
    MakeFromSave = () =>{
        ...
        console.log("printing the board from the save")
        console.log(this.props.navigation.state.params.freshBoard)
    }
    ...
    MakeSaveBoard(){
        ...
        for (let i = 0; i < (this.state.size); i++) {
            ...
            for (let j = 0; j < (this.state.size); j++) {
                console.log("check the cellArr" + " "+ cellArr.charAt((j)*2))
                cellArr.charAt((j)*2) == 1 ? row.push(1) : //row.push(0)
                (cellArr.charAt((j)*2) == 2 ? row.push(2) : row.push(0))   
            }
            SaveBoard.push(row)
        }
    }
        ...
        if(!this.state.win){
            for (let i = 0; i < (this.state.size + 1); i++) {
             ...
             for (let j = 0; j < (this.state.size + 1); j++) {
                ...
                if ( i!== 0 && j !==0){
                    let Boardcellval = new Array(this.state.Board[j-1]).toString().charAt((i-1)*2)
                    let solvedBoardcellval = new Array(this.state.solvedBoard[j-1]).toString().charAt((i-1)*2)
                    row.push(
                        <GameCell
                              cellSize = {cell} 
                              backProp = {(!this.state.isGenerated && Boardcellval == 2 && solvedBoardcellval != 2 && this.state.hint) ? '#ff0000' : '#00ff001C'}
                              solvedBoardcellval = {solvedBoardcellval}
                              Boardcellval = {Boardcellval}
                              key = {lst_key++}
                        ...
                        <TouchableOpacity 
                            ...
                            onPress={() => {
                                ...
                                SaveGame(this.state.name,
                                        (d.getHours().toString()+ ":" + d.getMinutes().toString()+":"+d.getSeconds().toString()+" "+ d.getDate().toString() + "/" + d.getMonth().toString() + "/" + d.getFullYear().toString()),
                                        this.state.size,
                                        this.state.density,
                                        SaveBoard,
                                        tentCount,
                    ...
                    <View style={{flex:3, flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center'}}>
                        <TouchableOpacity 
                            ...
                            onPress={() => {
                                ...
                                clearStats(statistics)
                                tentCount = this.state.isGenerated ? 0 : this.props.navigation.state.params.tentCount
                            } }
                                >
